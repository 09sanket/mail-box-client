...
const MailItem = ({ item }) => {
  ...
  const MailDeleteHandler = async () => {
    try {
      const response = await axios.patch(
        `https://mail-box-1-d23c1-default-rtdb.firebaseio.com/mailcompose/${item._id}.json`,
        { isDeleted: true }
      );
      dispatch(globalActions.deleteMail(item._id));
  ...
  return (
    ...
    >
      ...
      <div className='text-4xl text-gray-500 p-2' onClick={MailDeleteHandler}>
        <AiOutlineDelete />
      </div>
    </div>
  );
